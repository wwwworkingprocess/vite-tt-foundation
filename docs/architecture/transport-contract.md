...
packages/simulation
  ...
  revisions/ticks, deterministic application, snapshots
```
...
4. `revision` is the authoritative state version. `tick` is simulation time. They are related but not interchangeable.
...
- Message listener ordering matches the order accepted by the client adapter. Consumers still verify offsets and revisions.
...
type ClientLifecycle =
  ...
  | { state: 'ready'; revision: Revision; tick: SimulationTick }
  | { state: 'degraded'; revision?: Revision; reason: DegradedReason }
...
type EventOffset = number;
```

Values must eventually be runtime-validated. `Revision`, `SimulationTick`, and `EventOffset` are non-negative integers. Revision and offset are monotonic within their documented scope.
...
5. compare `expectedRevision` when supplied;
6. validate the domain command;
7. apply it atomically with idempotency recording;
8. assign the resulting revision and terminal result;
...
An `applied` result proves exact-once application for that command ID and names the resulting revision. It does not prove that any subscriber has processed the associated update.

A `rejected` result is also terminal and is stored for duplicate handling. Rejection does not advance revision. `duplicate: true` means the stored original result was returned.
...
- `offset` establishes contiguous observation order for the client-visible stream.
- Multiple events may share a revision when one command produces several facts; their offsets remain distinct.
- Events with an already applied `eventId` or offset are duplicates and are ignored after compatibility checks.
- An offset greater than `lastAppliedOffset + 1` is a gap. The client pauses incremental application and synchronizes.
- An event with a revision older than the current full read model is stale unless the synchronization protocol explicitly identifies it as replay metadata.
...
interface FullReadModelEnvelope<TReadModel> {
  ...
  revision: Revision;
  tick: SimulationTick;
  lastIncludedOffset: EventOffset;
...
type SynchronizationResponse<TReadModel, TMessage> =
  | {
      ...
      gameId: GameId;
      mode: 'delta';
      fromExclusiveOffset: EventOffset;
      throughOffset: EventOffset;
...
The host returns `delta` only when it retains a complete contiguous sequence after the client's offset and the client's baseline/schema is compatible. Otherwise it returns `full`.
...
4. set the resulting revision, tick, and offset;
...
| Browser tab refresh                                | New application session has no live transport state; persisted baseline may be present.                                | Treat as a new connection; validate logical session independently of `socket.id`.                       | Connect and send last trusted revision/offset if retained; otherwise request full baseline.                                                  | Conditional                                          |
| Duplicate command delivery                         | Existing `(gameId, commandId)` record with equivalent command fingerprint.                                             | Return the stored terminal result with `duplicate: true`; do not reapply.                               | Accept the result and reconcile to its revision.                                                                                             | No, unless client state has a gap                    |
| Acknowledgement timeout after host applied command | Transport timeout gives unknown outcome; retry finds stored command result.                                            | On same-ID retry/query, return original applied result.                                                 | Never invent a new ID automatically; retry with same ID, then synchronize if the resulting revision/update is missing.                       | Conditional                                          |
| Out-of-order or duplicated event observation       | Offset lower than/equal to applied offset is duplicate; offset higher than next expected is out of order/gap.          | Continue authoritative order; answer synchronization request.                                           | Ignore known duplicate; buffer only within bounded policy; pause and synchronize on unresolved ordering.                                     | Conditional                                          |
| Missed event gap                                   | Incoming offset is greater than `lastAppliedOffset + 1`, or revision transition is inconsistent.                       | Return complete delta if retained, otherwise current full read model.                                   | Stop incremental application and request synchronization.                                                                                    | Conditional                                          |
| Stale expected revision                            | Command `expectedRevision` differs from current revision.                                                              | Return terminal `stale_revision`; do not apply or advance revision.                                     | Synchronize, show/reconcile changed state, and require deliberate resubmission with a new command ID if intent remains valid.                | Conditional                                          |
...
| Client behind authoritative revision               | Handshake/sync comparison shows client revision/offset lower.                                                          | Supply contiguous delta when available and compatible, otherwise full baseline.                         | Apply verified delta or atomically replace baseline.                                                                                         | Conditional                                          |
| Client ahead of authoritative revision             | Client reports revision/offset greater than host's current state, often after rollback/restart or wrong game identity. | Reject delta assumption and return current full baseline plus diagnostic reason.                        | Verify game identity, discard local authoritative projection, and replace from host; surface serious consistency diagnostics.                | Yes                                                  |
